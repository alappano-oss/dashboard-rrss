...
def ingest_files(files, forced_brand: str|None=None) -> dict:
    ...
    for file in files:
        ...
        try:
            ...
            result['warnings'] += [f'{name}: {w}' for w in validation.warnings]
            if not validation.valid:
                result['rejected'] += 1; result['errors'] += [f'{name}: {e}' for e in validation.errors]; continue
            brand=forced_brand or infer_brand(name, raw)
            if brand is None:
                result['rejected'] += 1; result['errors'].append(f'{name}: no se pudo identificar la marca automáticamente.'); continue
            clean=normalize_dataframe(raw,name,brand)
            by_brand.setdefault(brand,[]).append(clean)
            result['files_processed'] += 1
        except Exception as exc:
            result['rejected'] += 1; result['errors'].append(f'{name}: {exc}')
...
def ingest_raw_directory(brand: str) -> dict:
    ...
    if not files:
        return {'files_processed':0,'new_records':0,'duplicates':0,'rejected':0,'errors':[f'No hay CSV en {folder}.'],'warnings':[]}
